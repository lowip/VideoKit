...
class VideoPlayerModel {
  ...
  func prepareForPlayback() {
    ...
    self.playerItem.asset.loadValuesAsynchronously(forKeys: VideoPlayerModel.keysForPlayback) {
      [weak self] in
      ...
      let (success, keyError) = VideoPlayerModel.keysForPlayback.reduce((true, nil)) {
        tuple, key -> (Bool, Error?) in
  ...
  private func setupTimeObserver(player: AVPlayer?, previousPlayer: AVPlayer?) {
    ...
    self.timeObserver = player.addPeriodicTimeObserver(
      ...
      guard self.totalSeconds != 0, !self.totalSeconds.isNaN,
  ...
  private func setupPlayerItemObservers(
    playerItem: AVPlayerItem?, previousPlayerItem: AVPlayerItem?
  ) {

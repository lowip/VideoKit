...
extension URL {
  ...
  /**
   ...
   For instance `http://domain.ext/media` is replaced by `videokit+http://domain.ext/media+videokit.mp4`.
  ...
  func wrap() -> URL? {
    ...
    components.path = self.pathExtension == ""
  ...
  /**
   ...
   For instance `videokit+http://domain.ext/media+videokit.mp4` is replaced by `http://domain.ext/media`.
  ...
  func unwrap() -> URL? {
    // Retrieve the url components and scheme
    guard var components = URLComponents(url: self, resolvingAgainstBaseURL: false),
      let wrappedScheme = components.scheme else {
        return nil
    }

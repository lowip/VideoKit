...
final class VideoPlayer {
  ...
  init(url: URL? = nil, cacheKey: String? = nil) {
    ...
    let visibilityDisplayLink = CADisplayLink(target: WeakProxy(self), selector: #selector(self.updateCurrentPlayerView))
  ...
  @objc private func updateCurrentPlayerView() {
    ...
    let (currentPlayerView, _) = self.playerViews.allObjects
  ...
  private func setupObservers() {
    ...
    self.processQueue.async {
      ...
      self.playerObservations = [
        ...
        self.player.observe(\.timeControlStatus, options: [.initial, .new]) { [weak self] player, change in
          ...
          case .waitingToPlayAtSpecifiedRate:
            guard let reason = player.reasonForWaitingToPlay,
              reason == .toMinimizeStalls else { break }
...
extension VideoPlayer: VideoPlayerPlaybackProtocol {
  ...
  func seek(to time: CMTime, forceResume: Bool) -> Bool {
    ...
    guard self.playerStatus != .unknown, self.playerStatus != .failed, self.playerStatus != .stopped else {
      return false
    }
  ...
  func pause() {
    ...
    self.processQueue.async { [weak self] in
      guard let `self` = self,
        self.requestedState == .paused,
        self.playerStatus != .paused else { return }
  ...
  func play() {
    ...
    self.processQueue.async { [weak self] in
      guard let `self` = self,
        self.requestedState == .playing else { return }
  ...
  func stop() {
    ...
    self.processQueue.async { [weak self] in
      guard let `self` = self,
        self.requestedState == .stopped,
        self.playerStatus != .stopped else { return }
